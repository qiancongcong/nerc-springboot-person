...
if ssh $ssh_transfer test -e $transferfile; then
    ...
    scp -o "StrictHostKeyChecking no" scripts/deploy_beta/deploy_cp2producttarget.sh $transferserver:/deploy/nerc-scripts/
    scp -o "StrictHostKeyChecking no" scripts/deploy_beta/deploy_target.sh $transferserver:/deploy/nerc-scripts/
    scp -o "StrictHostKeyChecking no" scripts/deploy_beta/deploy_on_server.sh $transferserver:/deploy/nerc-scripts/
